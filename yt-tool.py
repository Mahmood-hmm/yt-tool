...
while True :
    ...
    if choice == "1" : 
        ...
        if quality == "1" :
            os.system('yt-dlp -f "bv*[vcodec^=avc1][height<=2160]+[acodec^=mp4a]/b [height<=2160]" --merge-output-format mp4 -o "%(title)s.mp4" ' + url)
            after_dow(url)
            input (colored("Enter to continue" , "red"))
        elif quality == "2" :
            os.system('yt-dlp -f "bv*[vcodec^=avc1][height<=1440]+[acodec^=mp4a]/b [height<=1440]" --merge-output-format mp4 -o "%(title)s.mp4" ' + url)
            after_dow(url)
            input (colored("Enter to continue" , "red"))
        elif quality == "3" :
            os.system('yt-dlp -f "bv*[vcodec^=avc1][height<=1080]+[acodec^=mp4a]/b [height<=1080]" --merge-output-format mp4 -o "%(title)s.mp4" ' + url)
            after_dow(url)
            input (colored("Enter to continue" , "red"))
        elif quality == "4" :
            os.system('yt-dlp -f "bv*[vcodec^=avc1][height<=720]+[acodec^=mp4a]/b [height<=720]" --merge-output-format mp4 -o "%(title)s.mp4" ' + url)
            after_dow(url)
            input (colored("Enter to continue" , "red"))
        elif quality == "5" : 
            os.system('yt-dlp -f "bv*[vcodec^=avc1][height<=480]+[acodec^=mp4a]/b [height<=480]" --merge-output-format mp4 -o "%(title)s.mp4" ' + url)
            after_dow(url)
            input (colored("Enter to continue" , "red"))
    elif choice == "2" :
        ...
        if file_choice.lower() == "yes" :
            ...
            if os.path.exists(name):
                print(colored("The file exists already !", "green"))
            if quality == "1" :
                os.system('yt-dlp -f "bv*[vcodec^=avc1][height<=2160]+[acodec^=mp4a]/b [height<=2160]" --merge-output-format mp4 -o "%(playlist_title)s/%(title)s.mp4" ' + url)
                after_dow(url)
                input (colored("Enter to continue" , "red"))
            elif quality == "2" :
                os.system('yt-dlp -f "bv*[vcodec^=avc1][height<=1440]+[acodec^=mp4a]/b [height<=1440]" -o "%(playlist_title)s/%(title)s.mp4" ' + url)
                after_dow(url)
                input (colored("Enter to continue" , "red"))
            elif quality == "3" :
                os.system('yt-dlp -f "bv*[vcodec^=avc1][height<=1080]+[acodec^=mp4a]/b [height<=1080]" -o "%(playlist_title)s/%(title)s.mp4" ' + url )
                after_dow(url)
                input (colored("Enter to continue" , "red"))
            elif quality == "4" :
                os.system('yt-dlp -f "bv*[vcodec^=avc1][height<=720]+[acodec^=mp4a]/b [height<=720]" -o "%(playlist_title)s/%(title)s.mp4" ' + url)
                after_dow(url)
                input (colored("Enter to continue" , "red"))
            elif quality == "5" : 
                os.system('yt-dlp -f "bv*[vcodec^=avc1][height<=480]+[acodec^=mp4a]/b [height<=480]" -o "%(playlist_title)s/%(title)s.mp4" ' + url)
                after_dow(url)
                input (colored("Enter to continue" , "red"))
